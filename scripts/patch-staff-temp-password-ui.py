...
old = """async function createStaffScoped(name,email,role,prodi=''){
...
new = """function showStaffCredential(data,name,role,prodi=''){
...
if old not in s:
    raise SystemExit('createStaffScoped block not found')
s = s.replace(old, new, 1)

needle = "  if(action==='close-staff')return $('#staffModal')?.remove()\n  if(action==='reset-user')return resetUser(el.dataset.email)"
replacement = """  if(action==='close-staff')return $('#staffModal')?.remove()
...
if needle not in s:
    raise SystemExit('handleClick insertion point not found')
