...
async function doLogin(){
  ...
  loading(true,'Memverifikasi akun...')
  try{
    ...
    state.page=isAdmin()?'admin-dashboard':'student-dashboard'
    await renderPrivate()
    toast('Login berhasil.')
  }catch(e){
    box.textContent=e.message||'Login gagal.'
...
async function logout(){
  ...
  try{
    ...
    state.user=null;state.profile=null;state.selectedProdi=''
    await renderLanding()
  }finally{loading(false)}
}
...
async function savePassword(){
  ...
  try{
    ...
    show('Password berhasil diperbarui.',false)
    setTimeout(()=>{const m=$('#passwordModal');m.classList.add('hidden');m.classList.remove('flex')},500)
    toast('Password baru aktif.')
...
async function handleClick(e){
  ...
  if(action==='choose-prodi'){
    ...
    state.selectedProdi=p
    $('#privateMain').innerHTML=seminarHtml()
    validateSeminar()
    return
  }
...
async function handleChange(e){
  ...
  if(t.matches('[data-action="set-status"]')){
    const r=await supabaseClient.from('pendaftaran_seminar').update({status:t.value}).eq('id',t.dataset.id)
    if(r.error)toast(r.error.message,'err');else{toast('Status pendaftaran diperbarui.');await loadRegistrations()}
    return
  }
  if(t.matches('[data-action="set-role"]')){
    try{await edge('admin-users',{action:'set_role',user_id:t.dataset.id,role:t.value});toast('Role user diperbarui.');await loadUsers()}catch(err){toast(err.message,'err')}
    return
  }
  if(t.id==='importFile'){
    const f=t.files?.[0];if(!f)return
    $('#importFileName').textContent=f.name
    loading(true,'Membaca data mahasiswa...')
  ...
  if(t.matches('.doc-input')){
    ...
    badge.className='doc-badge self-center rounded-full bg-emerald-50 px-2 py-1 text-[10px] font-bold text-emerald-700'
    row.querySelector('[data-action="remove-doc"]').classList.remove('hidden')
...
async function handleSubmit(e){
  ...
  if(e.target.id==='seminarForm'){
    e.preventDefault()
    const ok=await submitSeminar()
    if(ok){state.selectedProdi=normalizeProdi(state.profile?.prodi||'');$('#privateMain').innerHTML=seminarHtml();validateSeminar()}
    return
  }
